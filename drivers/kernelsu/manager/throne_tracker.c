...
static DEFINE_MUTEX(ksu_throne_lock);
...
static bool crown_manager(const char *apk, struct list_head *uid_data,
                          uid_t preferred_uid)
{
    ...
    list_for_each_entry (np, list, list) {
        if (strncmp(np->package, pkg, KSU_MAX_PACKAGE_NAME) == 0) {
            if (preferred_uid != KSU_INVALID_APPID && np->uid != preferred_uid)
                return false;
                    ...
                    uid_t preferred_uid)
{
    ...
    for (i = depth; i >= 0; i--) {
        ...
        list_for_each_entry_safe (pos, n, &data_path_list, list) {
            ...
            if (crown_manager(candidate_path, uid_data, preferred_uid))
                stop = 1;
...
static bool __track_throne(bool prune_only, uid_t preferred_uid)
{
    ...
    if (!manager_exist) {
        ...
        search_manager("/data/app", 2, &uid_list, preferred_uid);
...
out:
    ...
    if (preferred_uid == KSU_INVALID_APPID)
        return ksu_is_manager_appid_valid();

    return is_uid_manager(preferred_uid);
}

void track_throne(bool prune_only)
{
    mutex_lock(&ksu_throne_lock);
    __track_throne(prune_only, KSU_INVALID_APPID);
    mutex_unlock(&ksu_throne_lock);
}
...
bool track_throne_for_uid(uid_t uid)
{
    ...
    uid %= KSU_PER_USER_RANGE;
    mutex_lock(&ksu_throne_lock);
    claimed = __track_throne(false, uid);
    mutex_unlock(&ksu_throne_lock);

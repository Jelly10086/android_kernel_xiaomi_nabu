...
vmlinux_link()
{
	...
	if [ "${SRCARCH}" != "um" ]; then
		...
		${ld} ${ldflags} -o ${output} -T ${lds} ${objects}
	else
		if [ -n "${CONFIG_THIN_ARCHIVES}" ]; then
...
# Generate .BTF type information from DWARF debug information.
# ${1} - vmlinux image
# ${2} - relocatable object containing only the BTF section
gen_btf()
{
	local pahole_ver
	...
	# Keep only allocatable BTF data and turn the image into an ET_REL input.
